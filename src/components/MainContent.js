...
import usePreferences from "../hooks/usePreferences"; // <- default import

const MainContent = () => {
  // usePreferences default hook: [preferences, updatePreferences, { loading }]
  const [preferences = {}, updatePreferences] = usePreferences();

  // local UI state — initialize with sensible defaults
  ...
  const handleAction = (action) => {
    ...
    const normalized = key === "customize" ? "customise" : key;
  ...
  // ---- Sync preferences -> local state when preferences load or change ----
  useEffect(() => {
    // preferences may be undefined initially
  ...
  // ---- Load meshes from PetModel when user opens customise panel ----
  useEffect(() => {
    if (activePanel !== "customise") return;
    // if we already have meshes loaded, skip
        ...
        if (petRef.current) {
          if (typeof petRef.current.getMeshes === "function") {
            ...
            if (Array.isArray(remote) && remote.length) {
              ...
              // persist to preferences (store full array)
  ...
  const handleDrop = (e) => {
    ...
    if (food) {
      ...
      // compute merged stats safely (don't overwrite other stats)
      const prevStats = preferences.stats || {};
      const newStats = {
        ...prevStats,
        fedCount: (prevStats.fedCount || 0) + 1,
      };
  ...
  // ---- Color change (update local + prefs + model) ----
  const handleColorChange = (meshName, color) => {
    setMeshes((prev) => {
      const updated = prev.map((m) => (m.name === meshName ? { ...m, color } : m));
      // persist the entire meshes array (merge handled by updatePreferences)
  ...
  // ---- Apply scene (teleport) ----
  const applyScene = (scene) => {
    // store scene object (name + img URL). imported images are strings at build time.
  ...
  // ---- small helper to compute stats for display (unchanged) ----
